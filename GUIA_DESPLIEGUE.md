...
## 4. Despliegue del Frontend y Backend en Render

### Opción Unificada (Recomendada)
El proyecto empaqueta la SPA de React directamente dentro del ejecutable JAR de Spring Boot (`src/main/resources/static`). Al desplegar el servicio Web Java en Render con `render.yaml`, un único servicio sirve tanto la interfaz gráfica como la API REST.

### Opción Desacoplada (Render Static Site)
Si se desea desplegar el frontend como un servicio estático independiente en Render:
1. **New → Static Site**, mismo repositorio.
2. Build command: `npm install && npm run build`.
3. Publish directory: `src/main/resources/static`.
4. Variable de entorno: `VITE_API_URL=https://<tu-backend>.onrender.com`.
5. Regla de reescritura: `/* → /index.html` (definida en `render.yaml`) para el enrutamiento SPA.

### Configuración CORS
Una vez desplegado el frontend, actualiza `FRONTEND_ORIGINS` en las variables de entorno del backend en Render con la URL pública real del frontend.
...
- [ ] `GET https://<backend>.onrender.com/error` responde.
- [ ] `POST https://<backend>.onrender.com/usuarios` con un body válido responde `201`.
- [ ] `POST https://<backend>.onrender.com/auth/login` responde `200` con un `token`.
- [ ] El frontend desplegado en Render permite registrarse, inicia sesión automáticamente y redirige a `/perfil`.
